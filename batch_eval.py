...
from mmaudiosep.data.data_setup import setup_eval_dataset
from mmaudiosep.eval_utils import ModelConfig, all_model_cfg, separate
...
def main(cfg: DictConfig):
    ...
    net: MMAudioSep = get_my_mmaudiosep(cfg.model).to(device).eval()
    ...
    with torch.amp.autocast(enabled=cfg.amp, dtype=torch.bfloat16, device_type=device):
        for batch in tqdm(loader):
            audios = separate(batch.get('clip_video', None),
                              batch.get('sync_video', None),
                              batch.get('caption', None),
                              batch.get('mixture', None),
