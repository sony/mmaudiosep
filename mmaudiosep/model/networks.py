...
class MMAudioSep(nn.Module):
    ...
    def __init__(self,
        ...
        if v2:
            self.audio_input_proj = nn.Sequential(
                ChannelLastConv1d(latent_dim*2, hidden_dim, kernel_size=7, padding=3), # [Note] concate mixture latents
                nn.SiLU(),
    ...
    def preprocess_conditions(self, clip_f: torch.Tensor, sync_f: torch.Tensor,
                              text_f: torch.Tensor, x_cond: torch.Tensor) -> PreprocessedConditions:
        """
    ...
    def predict_flow(self, latent: torch.Tensor, t: torch.Tensor,
        ...
        x_cond = conditions.x_cond

        # (small_16k) [bs, frame, 20] -> [bs, frame, 40] -> [bs, frame, 448]
        # (large_44k) [bs, frame, 40] -> [bs, frame, 80] -> [bs, frame, 896]
        latent=torch.cat([latent, x_cond], dim=2)
    ...
    def forward(self, latent: torch.Tensor, clip_f: torch.Tensor, sync_f: torch.Tensor,
                text_f: torch.Tensor, x_cond: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """
        latent: (B, N, C) 
        vf: (B, T, C_V)
        t: (B,)
        """
        conditions = self.preprocess_conditions(clip_f, sync_f, text_f, x_cond)
    ...
    def get_empty_conditions(
        ...
        conditions = self.preprocess_conditions(empty_clip, empty_sync, empty_text, x_cond)
    ...
    def load_weights_from_mmaudio(self, src_dict) -> None:
        if 't_embed.freqs' in src_dict:
            del src_dict['t_embed.freqs']
        if 'latent_rot' in src_dict:
            del src_dict['latent_rot']
        ...
        # [Note] we changed audio input proj. so remove it from original mmaudio model.
        if 'audio_input_proj.0.weight' in src_dict:
            del src_dict['audio_input_proj.0.weight']
            del src_dict['audio_input_proj.0.bias']
            del src_dict['audio_input_proj.2.w1.weight']
            del src_dict['audio_input_proj.2.w2.weight']
            del src_dict['audio_input_proj.2.w3.weight']

        self.load_state_dict(src_dict, strict=False) # if network is changed, please set strict=True
...
def small_16k(**kwargs) -> MMAudioSep:
    num_heads = 7
    return MMAudioSep(latent_dim=20,
                   clip_dim=1024,
                   sync_dim=768,
                   text_dim=1024,
...
def small_44k(**kwargs) -> MMAudioSep:
    num_heads = 7
    return MMAudioSep(latent_dim=40,
                   clip_dim=1024,
                   sync_dim=768,
                   text_dim=1024,
                   hidden_dim=64 * num_heads,
...
def medium_44k(**kwargs) -> MMAudioSep:
    num_heads = 14
    return MMAudioSep(latent_dim=40,
                   clip_dim=1024,
                   sync_dim=768,
...
def large_44k(**kwargs) -> MMAudioSep:
    num_heads = 14
    return MMAudioSep(latent_dim=40,
                   clip_dim=1024,
                   sync_dim=768,
...
def large_44k_v2(**kwargs) -> MMAudioSep:
    num_heads = 14
    return MMAudioSep(latent_dim=40,
                   clip_dim=1024,
                   sync_dim=768,
...
def get_my_mmaudiosep(name: str, **kwargs) -> MMAudioSep:
    if name == 'small_16k':
...
if __name__ == '__main__':
    network = get_my_mmaudiosep('small_16k')
