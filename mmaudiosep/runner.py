...
from mmaudiosep.model.networks import get_my_mmaudiosep
...
class Runner:
    ...
    def __init__(self,
        ...
        self.network = DDP(get_my_mmaudiosep(cfg.model,
        ...
        if mode == '16k':
            self.features = FeaturesUtils(
                ...
                mode=mode,
                need_vae_encoder=True, # for separation, need vae encoder
            )
        elif mode == '44k':
            self.features = FeaturesUtils(
        ...
        if for_training:
            ...
            info_if_rank_zero(self.log, f'Total parameters: {total_params / 1e6:.2f}M')
            info_if_rank_zero(self.log, f'Trainable parameters: {trainable_params / 1e6:.2f}M')
    ...
    def train_fn(
        ...
        pred_v = self.network(xt, clip_f, sync_f, text_f, x_cond, t)
        loss = self.fm.loss(pred_v, x0, x1)
        mean_loss = loss.mean()
        return x1, loss, mean_loss, t, x_cond
    ...
    def train_pass(self, data, it: int = 0):
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            a_dist = self.features.encode_audio(audio)
            a_mean = a_dist.mean.detach().transpose(1, 2)
            a_std  = a_dist.std.detach().transpose(1, 2)

            mixture = data['mixture'].cuda(non_blocking=True)
            mix_dist = self.features.encode_audio(mixture)
            mix_mean = mix_dist.mean.detach().transpose(1, 2)
            mix_std  = mix_dist.std.detach().transpose(1, 2)
            ...
            x1, loss, mean_loss, t, x_cond = self.train_fn(clip_f, sync_f, text_f, a_mean, a_std, mix_mean, mix_std)
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp,
            ...
            try:
                if it % self.log_extra_interval == 0:
                    ...
                    x_cond=x_cond[0:1]
                    conditions = self.network.module.preprocess_conditions(clip_f, sync_f, text_f, x_cond)
                    empty_conditions = self.network.module.get_empty_conditions(x0.shape[0], x_cond)
                    ...
                    self.log.log_audio('train',
                                       ...
                                       sample_rate=self.sample_rate)
                    # save mixture
                    x_cond = self.network.module.unnormalize(x_cond)
                    mel = self.features.decode(x_cond)
                    audio = self.features.vocode(mel).cpu()[0]  # 1 * num_samples
                    self.log.log_spectrogram('train', f'spec-gt-mix-r{local_rank}', mel.cpu()[0], it)
                    self.log.log_audio('train',
                                       f'audio-gt-mix-r{local_rank}',
                                       audio,
                                       it,
                                       sample_rate=self.sample_rate)
    ...
    def validation_pass(self, data, it: int = 0):
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            self.log.data_timer.end()
            loss, mean_loss, t = self.val_fn(clip_f.clone(), sync_f.clone(), text_f.clone(), x1, x_cond.clone())
    ...
    def inference_pass(self,
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            a_randn = torch.empty_like(mix_mean).normal_(generator=self.rng) # shape from mixture
            x_cond = mix_mean + mix_std * a_randn
            ...
            # sample
            x0 = torch.empty_like(mix_mean).normal_(generator=self.rng)
            conditions = self.network.module.preprocess_conditions(clip_f, sync_f, text_f, x_cond)
            empty_conditions = self.network.module.get_empty_conditions(x0.shape[0], x_cond)
            cfg_ode_wrapper = lambda t, x: self.network.module.ode_wrapper(
                t, x, conditions, empty_conditions, self.cfg_strength)
    ...
    def load_weights_in_memory_from_mmaudio(self, src_dict):
        self.network.module.load_weights_from_mmaudio(src_dict)
        self.log.info('Network weights loaded from memory.')

    def load_weights_from_mmaudio(self, path):
        # This method loads only the network weight and should be used to load a pretrained model
        map_location = 'cuda:%d' % local_rank
        src_dict = torch.load(path, map_location={'cuda:0': map_location}, weights_only=True)

        self.log.info(f'Importing network weights from {path}...')
        self.load_weights_in_memory_from_mmaudio(src_dict)

    def freeze_params_from_mmaudio(self, is_freeze: bool = True) :
        total_params = sum(p.numel() for p in self.network.module.parameters() if p.requires_grad)
        self.log.info(f"original learnable parameters: {total_params}") # for debug

        # except audio input proj.
        param_prefixes = [
            "empty_clip_feat",
            "empty_sync_feat",
            "sync_pos_emb",
        ...
        for name, param in self.network.module.named_parameters():
            if any(name.startswith(prefix) for prefix in param_prefixes):
                param.requires_grad = False

        # freeze Single-DIT & Final block
        for name, param in self.network.module.named_parameters():
            if "fused_blocks" in name or "final_layer" in name:
                param.requires_grad = False
        ...
        if freeze_mode == 2 :
            for name, param in self.network.module.named_parameters():
                if "joint_blocks" in name and ("text_block" in name or "clip_block" in name):
                    param.requires_grad = False
        elif freeze_mode == 3 :
            for name, param in self.network.module.named_parameters():
                if "joint_blocks" in name:
                    param.requires_grad = False
        elif freeze_mode == 4:
            for name, param in self.network.module.named_parameters():
                if "fused_blocks" in name or "final_layer" in name:
        ...
        total_params = sum(p.numel() for p in self.network.module.parameters() if p.requires_grad)
        self.log.info(f"the number of learnable parameters : {total_params}")
        if local_rank == 0 :
            for name, param in self.network.module.named_parameters(): # debug
