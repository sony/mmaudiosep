...
class ExtractedVGG(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        if mode == '44k' :
            self.audio_len=353280
        else :
            self.audio_len=128000
        # SNR configuration
        self.is_snr_set=is_snr_set
        if is_snr_set == True:
            self.snr_range=max_snr-min_snr
    ...
    # for debug
    def calculate_snr(self, audio: torch.Tensor, noise: torch.Tensor) -> float:
    ...
    def compute_noise_scaling_factor(self, audio: torch.Tensor, noise: torch.Tensor, target_snr_db: float) -> float:
        signal_power = torch.mean(audio ** 2)
        noise_power = torch.mean(noise ** 2)
        eps=1e-6
        target_ratio = 10 ** (target_snr_db / 10)
        scaling_factor = torch.sqrt(signal_power / (target_ratio * (noise_power+eps)))
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        audio_path = f'{self.audio_path}/{id}.flac'
        audio, fs = torchaudio.load(audio_path)
        audio = audio[:, :self.audio_len]

        dataset_len = len(self.df_list)
        while True:
            noise_idx = random.choice([i for i in range(dataset_len) if i != idx])
            if self.df_list[idx]['label'] != self.df_list[noise_idx]['label']:
                break

        noise_id = self.df_list[noise_idx]['id']
        noise_path = f'{self.audio_path}/{noise_id}.flac'
        noise, fs = torchaudio.load(noise_path)
        noise_len = noise.shape[1]

        idx_start = random.randint(0, max(0, noise_len - self.audio_len))
        noise = noise[:, idx_start:idx_start + self.audio_len]

        if self.is_snr_set : 
            target_snr= torch.rand(1) * self.snr_range + self.min_snr # -15~-15dB
            scaling_factor=self.compute_noise_scaling_factor(audio, noise, target_snr)
        ...
        if max_val > 1.0 :
            ...
            coeff = 1.
        audio = audio.squeeze() * coeff
        mixture = mixture.squeeze() * coeff
