...
from mmaudiosep.data.extracted_audio_sep import ExtractedAudio
from mmaudiosep.data.extracted_vgg_sep import ExtractedVGG
from mmaudiosep.data.eval.video_dataset_gen import VGGSound
from mmaudiosep.data.eval.video_dataset_sep import Mixed

from mmaudiosep.data.mm_dataset import MultiModalDataset
...
def load_vgg_data(cfg: DictConfig, data_cfg: DictConfig) -> Dataset:
    ...
    dataset = ExtractedVGG(tsv_path=data_cfg.tsv,
                           audio_path=data_cfg.audio_dir,
                           data_dim=cfg.data_dim,
                           premade_mmap_dir=data_cfg.memmap_dir,
                           mode=mode,
                           is_snr_set=cfg.use_snr_setting,
                           max_snr=cfg.max_snr,
                           min_snr=cfg.min_snr)
...
def setup_eval_dataset(dataset_name: str, cfg: DictConfig) -> tuple[Dataset, DataLoader]:
    ...
    if dataset_name.startswith('vggsound'): # for generation test from white noise
        dataset = VGGSound(cfg.eval_data.VGGSound.video_path,
                           cfg.eval_data.VGGSound.csv_path,
                           duration_sec=cfg.duration_s,
                           mode=mode)
    elif dataset_name.startswith('vggclean'):
        dataset = Mixed(cfg.eval_data.VGGSoundClean_test.video_path,
                        cfg.eval_data.VGGSoundClean_test.tsv_path,
                        cfg.eval_data.VGGSoundClean_test.audio_path,
                        duration_sec=cfg.duration_s,
                        mode=mode)
    elif dataset_name.startswith('music'):
        dataset = Mixed(cfg.eval_data.MUSIC_test.video_path,
                        cfg.eval_data.MUSIC_test.tsv_path,
                        cfg.eval_data.MUSIC_test.audio_path,
                        duration_sec=cfg.duration_s,
                        mode=mode)
    else:
