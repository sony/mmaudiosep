...
from mmaudiosep.data.data_setup import setup_training_datasets, setup_val_datasets
from mmaudiosep.runner import Runner
from mmaudiosep.sample import sample
from mmaudiosep.utils.synthesize_ema import synthesize_ema
...
def train(cfg: DictConfig):
    ...
    eval_cfg = compose('eval_config', overrides=[f'exp_id={cfg.exp_id}', f'model={cfg.model}', f'batch_size={cfg.eval_batch_size}'])
    eval_cfg.data = cfg.data
    ...
    with open_dict(cfg):
        ...
        eval_cfg.data_dim = cfg.data_dim # add
        ...
        if checkpoint is not None:
            ...
            if cfg['weights'] is not None:
                if cfg['freeze_params'] :
                    trainer.freeze_params_from_mmaudio(is_freeze=True)
                cfg['weights'] = None
            elif cfg['mmaudio_weights'] is not None:
                if cfg['freeze_params'] :
                    trainer.freeze_params_from_mmaudio(is_freeze=True)
                cfg['mmaudio_weights'] = None
        else:
            # load previous network weights if needed
            curr_iter = 0
            if cfg['weights'] is not None:
                info_if_rank_zero(log, 'Loading weights from the disk')
                trainer.load_weights(cfg['weights']) # load pretrained original mmaudio model
                if cfg['freeze_params'] :
                    trainer.freeze_params_from_mmaudio(is_freeze=True)
                cfg['weights'] = None
            elif cfg['mmaudio_weights'] is not None:
                info_if_rank_zero(log, 'Loading weights from the disk (MMAudio)')
                trainer.load_weights_from_mmaudio(cfg['mmaudio_weights']) # load pretrained original mmaudio model
                if cfg['freeze_params'] :
                    trainer.freeze_params_from_mmaudio(is_freeze=True)
                cfg['mmaudio_weights'] = None
