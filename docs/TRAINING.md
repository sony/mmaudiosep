...
We have placed a strong emphasis on optimizing the training process for speed.
Consequently, some pre-processing steps are required.

Before starting any training, the following steps must be completed:
...
The following hardware configuration is recommended for a smooth and efficient training experience. These are not minimum requirements.
...
`training/extract_video_training_latents_sep.py` extracts audio, video, and text features and save them as a `TensorDict` with a `.tsv` file containing metadata to `output_dir`.
Additionally, it extracts audio files to `output_audio_dir`, which are used to dynamically generate mixed audio signals during training.

To run this script, use the `torchrun` utility:

```bash
torchrun --standalone training/extract_video_training_latents_sep.py
```
...
- `output_audio_dir` -- where audio files (flac format) are saved.
...
2. Run `training/extract_audio_training_latents_sep.py` to extract each clip's audio and text features and save them as a `TensorDict` with a `.tsv` file containing metadata to `output_dir`. Additionally, it extracts audio files to `output_audio_dir`, which are used to dynamically generate mixed audio signals during training.
...
```bash
torchrun --standalone training/extract_audio_training_latents_sep.py
```
...
Note that these reference tsv files are the **outputs** of `extract_audio_training_latents_sep.py`, which means the `id` column might contain duplicate entries (one per clip). You can still use it as the `captions_tsv` input though -- the script will handle duplicates gracefully.
...
In our current setup, we use the *_corrected.tsv files to ensure data consistency and avoid caption mismatches. (ie, mismatch between audio and caption, see https://github.com/hkchengrex/MMAudio/issues/56).
...
For full training on the large model with eight GPUs, use the following command:

```bash
OMP_NUM_THREADS=4 torchrun --standalone --nproc_per_node=8 train.py exp_id=exp_1
```
...
For all models, specify `vgg_oversample_rate` to be `3` to reduce overfitting.
